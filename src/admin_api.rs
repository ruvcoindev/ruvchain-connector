...
pub struct RouterState {
    pub version: ruvchainctl::RouterVersion,
...
pub async fn connect(config: Config) -> Result<RouterState, ()> {
    ...
    for uri in &config.ruvchain_admin_listen {
        if let Some((protocol, address)) = uri.split_once("://") {
                ...
                Ok(socket) => {
                        ...
                        .filter(|p| **p != PeeringProtocol::Tcp)
                    {
                            ...
                            .any(|p| p == *protocol)
                        {
                            ...
                            warn!("If both peering nodes have no appropriate `ruvchain_listen` URI set in the config");
                        }
                    }
            ...
            warn!("Can't parse ruvchain admin socket address {uri}");
            continue;
        }
    }
    for (uri, err) in errs {
...
pub async fn monitor(
    config: Config,
    state: State,
    watch_sessions: watch::Sender<Vec<ruvchainctl::SessionEntry>>,
    watch_peers: watch::Sender<Vec<ruvchainctl::PeerEntry>>,
) -> Result<(), ()> {
    loop {
        {
        ...
        sleep(config.ruvchainctl_query_delay).await;
    }
}
